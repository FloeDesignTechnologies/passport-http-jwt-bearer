...
function Strategy(secretOrPublicKey, options, verify) {
  if (!secretOrPublicKey || (typeof secretOrPublicKey != 'string' && !(secretOrPublicKey instanceof Buffer))) {
    throw new TypeError('HTTPJwtBearerStrategy requires a string or buffer containing either the secret for HMAC algorithms, or the PEM encoded public key for RSA and ECDSA callback');
  }
  ...
  if (!verify) {
    throw new TypeError('HTTPJwtBearerStrategy requires a verify callback');
  }
 ...
 */
util.inherits(Strategy, HTTPBearerStrategy);
